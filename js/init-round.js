...
;
demo.spawn = function(d, e) {
    var b, c, a;
    if (particles.length >= MAX_PARTICLES) {
        pool.push(particles.shift())
    }
    b = pool.length ? pool.pop() : new Particle();
    b.init(d, e, random(5, 40));
    b.wander = random(0.5, 2);
    b.color = random(COLOURS);
    b.drag = random(0.9, 0.99);
    c = random(TWO_PI);
    a = random(2, 8);
    b.vx = sin(c) * a;
    b.vy = cos(c) * a;
